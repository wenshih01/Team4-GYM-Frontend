<template>
    <div>
      <div class="top-bar">
        ...
        <v-btn color="primary" @click="openNew">新增課程</v-btn>
      </div>
      ...
      >
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions-container">
            ...
            <v-btn icon color="red" size="small" @click="deleteDetail(item.courseID)">
  <v-icon>mdi-delete</v-icon>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field
            label="課程時數 (分鐘)"
            v-model.number="detail.durationMinutes"
            outlined
            clearable
            type="number"
          ></v-text-field>
  ...
  // 表格標題
  const headers = ref([
    { title: "課程id", value: "courseID" },
    //   { title: "方案 ID", value: "planId" },
    { title: "課程名稱", value: "courseName" },
    //   { title: "動作 ID", value: "actionId" },
    { title: "課程細節", value: "description" },
    { title: "課程時數", value: "durationMinutes" },
    // { title: "建立時間", value: "createdAt" },
    // { title: "更新時間", value: "updatedAt" },
  ...
  const fetchDetails = async () => {
    try {
      const response = await axios.get("http://localhost:8082/courses/getAll"); // 連接 PlanDetailController
...
const executeSearch = async () => {
  try {
    // 呼叫後端新建的 /courses/search API
    const response = await axios.get("http://localhost:8082/courses/search", {
      params: { keyword: search.value }, // 傳入關鍵字
    });
    details.value = response.data; // 更新前端表格資料
  } catch (error) {
  ...
  try {
    if (isEdit.value) {
      // 更新課程
      await axios.put(
        `http://localhost:8082/courses/updateCourse/${detail.value.courseID}`,
        detail.value
      );
    } else {
      // 新增課程，移除 courseID
      const { courseID, ...newCourse } = detail.value; // 排除 courseID
      const response = await axios.post("http://localhost:8082/courses/addCourse", newCourse);

      // 將後端返回的資料（包含自動生成的 courseID）加入列表
  ...
  const deleteDetail = async (id) => {
    try {
      if (confirm("確定要刪除此方案細則嗎？")) {
        await axios.delete(`http://localhost:8082/courses/deleteCourse/${id}`); // 刪除方案細則
