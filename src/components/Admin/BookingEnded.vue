<template>
    <div>
      ...
      <v-btn color="primary" @click="goToPreviousBooking">
        切換到未開始課程
</v-btn>
  ...
  try {
    ...
    bookings.value = response.data
      .filter(booking => new Date(booking.schedule.endTime) < now) // 只保留「已結束」的課程
      .sort((a, b) => new Date(b.schedule.endTime) - new Date(a.schedule.endTime)); // 按結束時間排序，最新的課程在最上面
  ...
  try {
    ...
    bookings.value = response.data
      .filter(booking => new Date(booking.schedule.endTime) < now) // 只保留「已結束」的課程
      .sort((a, b) => new Date(b.schedule.endTime) - new Date(a.schedule.endTime)); // 按結束時間排序
...
const goToPreviousBooking = () => {
  router.push("/admin/booking"); // 請替換成你剛剛的 Vue 路由名稱
};
