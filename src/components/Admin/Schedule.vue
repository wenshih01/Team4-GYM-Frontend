<template>
    <div>
      ...
      <br>
      <v-btn color="primary" @click="goToScheduleEnded">
       切換到已結束排程
      </v-btn>
  ...
  <template v-slot:[`item.actions`]="{ item }">
    <div class="actions-container">
      ...
      <v-btn
            ...
            <v-btn icon color="blue" size="small" @click="showBookingDetails(item.scheduleId)">
...
></v-select>
  ...
  <v-card class="dialog-card">
    <v-card-title class="dialog-title">已預約的會員</v-card-title>

    <v-card-text class="dialog-content">
      <v-list v-if="bookedUsers.length">
        <v-list-item v-for="user in bookedUsers" :key="user.id" class="user-list-item">
          <span class="user-name">{{ user.name }}</span> 
          <span class="user-username">({{ user.username }})</span>
        </v-list-item>
      </v-list>
      <p v-else class="text-center no-course-text">無人預約</p>
    </v-card-text>

    <v-card-actions>
      <v-btn color="orange" text @click="bookingDialog = false">關閉</v-btn>
    </v-card-actions>
  </v-card>
</v-dialog>
...
const bookedUsers = ref([]); // 存儲預約用戶的數據
  ...
  try {
    ...
    schedules.value = response.data
      .map(schedule => ({
        ...
        formattedStartTime: formatDateTime(schedule.startTime), // 格式化時間
        formattedEndTime: formatDateTime(schedule.endTime),
      }))
      .filter(schedule => schedule.startTime > now) // 過濾未開始的課程
...
const goToScheduleEnded = () => {
  router.push("/admin/scheduleEnded"); // ✅ 修改成 `scheduleEnded` 頁面
};


const showBookingDetails = async (scheduleId) => {
  try {
